...
- [[#soml-tooling][SOML Tooling]]
  ...
  - [[#soml-simplify][soml-simplify]]
    - [[#example-source-soml-schema][Example Source (SOML schema)]]
    - [[#example-target-simplified][Example Target (Simplified)]]
  - [[#tsv2owl][tsv2owl]]
    - [[#ontology-preamble][Ontology Preamble]]
    - [[#tsv2owl-options][tsv2owl Options]]
    - [[#tsv2owl-columns-and-features][tsv2owl Columns and Features]]
    - [[#rdf-replacement][RDF Replacement]]
    - [[#running-tsv2owl][Running tsv2owl]]
  - [[#soml2puml][soml2puml]]
...
*** Ontology Preamble
:PROPERTIES:
:CUSTOM_ID: ontology-preamble
:END:
Use an ontology preamble, eg like this ~otkg-preamble.ttl~.
Include all "system" prefixes that are shown after the newline (in particular, use ~s:~ instead of ~schema:~)!
#+begin_src ttl
...
- ~-O~: add ~owl:Restriction~ to bind props to classes (else it uses only ~s:domainIncludes~ on props)
- ~-t~: add ~rdfs:range owl:Thing~ to props with range ~iri~
    (this is needed for Metaphactory to be able to edit "free URLs").
  - Without this option, they are still mapped to ~owl:ObjectProperty~, but without any ~rangeIncludes~.
...
With the ~-l~ option, the following downgrading (simplification) is also done:
#+begin_src 
rdf:langString        xsd:string
rdf:Literal           xsd:string
#+end_src

*** tsv2owl Columns and Features
:PROPERTIES:
:CUSTOM_ID: tsv2owl-columns-and-features
:END:
...
- Emits ~s:domainIncludes~ for each prop to accommodate props listed under several classes.
  - Use option ~-O~ to also add ~owl:Restriction~ to each class.
    This binds the prop to the class, and shows the class-dependent cardinality of the prop
  - Use the update ~tsv2owl-domain-range.ru~ to convert ~domainIncludes~ to ~rdfs:domain~ for props listed only once.
- Emits ~s:rangeIncludes~ for each prop
  - Use the update ~tsv2owl-domain-range.ru~ to convert ~rangeIncludes~ to ~rdfs:range~ for props that always point to the same class or datatype
  - We strongly recommend that for all props, and indeed that is currently a restriction in SOML.
- Emits type ~owl:ObjectProperty~ vs ~owl:DatatypeProperty~ for each prop, depending on range:
  - If ~iri~ then ~ObjectProperty~
    (and with option ~-t~, add ~rdfs:range owl:Thing~)
...
*** Running tsv2owl
:PROPERTIES:
:CUSTOM_ID: running-tsv2owl
:END:
You can run the tool with a ~Makefile~ like this (see [[./tsv2owl][tsv2owl]]), feeding from a Google Sheet:
#+begin_src Makefile
ontology.ttl ::
	curl -Ls "https://docs.google.com/spreadsheets/d/.../export?format=tsv" | \
	  perl -Swill be emitted as  tsv2owl.pl -vocab s: -ontology otkg: | \
...
#+end_src
Metaphactory also needs the commented-out statement (~allValuesFrom~ pointing to the property range).
To add that, use the SPARQL update ~tsv2owl-allValuesFrom.ru~.
Replace ~riot~ with ~update~ (which is also part of Jena):
#+begin_src Makefile
ontology.ttl ::
	curl -Ls "https://docs.google.com/spreadsheets/d/.../export?format=tsv" | \
...
The tool emits ~s:domain/rangeIncludes~ (which are polymorphic, i.e. multi-valued).
If you need to emit ~rdfs:domain/range~ (which are monomorphic, i.e. single-valued),
use the SPARQL update ~tsv2owl-domain-range.ru~ that does this:
